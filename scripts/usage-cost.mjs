...
function priceOf(model, t) {
	...
	if (!r)
		throw new Error(
			`unknown model "${model}" — try one of: ${Object.keys(RATES).join(", ")}`,
		);
...
function printRates() {
	console.log(
		"Rate tables (USD per 1M tokens, early 2026 — edit RATES to update):\n",
	);
...
function printRow(model, t) {
	...
	console.log(`\n${r.label}  (${model})`);
	console.log(
		`  input        ${n(t.input).padStart(12)} tok   ${usd(c.input)}`,
	);
	console.log(
		`  output       ${n(t.output).padStart(12)} tok   ${usd(c.output)}`,
	);
	if (t.cacheWrite)
		console.log(
			`  cache write  ${n(t.cacheWrite).padStart(12)} tok   ${usd(c.cacheWrite)}`,
		);
	if (t.cacheRead)
		console.log(
			`  cache read   ${n(t.cacheRead).padStart(12)} tok   ${usd(c.cacheRead)}`,
		);
...
if (args.help) {
	console.log(
		"See header of scripts/usage-cost.mjs for usage. `--rates` lists models.",
	);
	process.exit(0);
}
...
if (!process.stdin.isTTY) {
	...
	process.stdin.on("end", () => {
		...
		try {
			...
			console.error(
				"stdin was not valid JSON. Pass --model for manual mode, or pipe `ccusage --json`.",
			);
			process.exit(1);
		}
		...
		if (!Array.isArray(days) || days.length === 0) {
			console.error(
				"No `daily`/`monthly` array found in JSON. Expected `ccusage --json` output.",
			);
			process.exit(1);
		}
		...
		for (const day of days) {
			...
			for (const b of breakdowns) {
				...
				if (!model) {
					console.log(`  (skipping unmapped model "${b.modelName}")`);
					continue;
				}
	...
	console.log(
		"Nothing to do. Pass --model ... or pipe `ccusage --json`. Use --rates to list models, --help for usage.",
	);
}
